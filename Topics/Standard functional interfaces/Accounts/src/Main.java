...
class Main {
    ...
    public static void printFilteredAccounts(List<Account> accounts) {
        Predicate<Account> p = x -> x.getBalance() > 0;
        List<Account> nonEmptyAccounts = filter(accounts, p);// write your code here
        p = x -> !x.isLocked() && x.getBalance() >= 100_000_000;
        List<Account> accountsWithTooMuchMoney = filter(accounts,p); // write your code here
